...
const Account = () => {
  ...
  const fetchData = async () => {
    ...
    if (collabDataRaw) {
      const collaborationBooks = collabDataRaw
        .map((item) => item.ebooks as any)
        .filter(Boolean)
        .filter((ebook) => {
          const authorNames = String(ebook.author || "")
            .split(",")
            .map((authorName) => authorName.trim())
            .filter(Boolean);

          return new Set(authorNames).size > 1;
        });

      setCollabBooks(
        Array.from(
          new Map(
            collaborationBooks.map((ebook) => [ebook.id, ebook])
          ).values()
        )
      );
    }
